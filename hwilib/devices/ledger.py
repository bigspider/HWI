...
class LedgerClient(HardwareWalletClient):
    ...
    def display_multisig_address(
        ...
        def is_valid_der_path(path: str | None):
            if path is None or path[:3] not in ["/0/", "/1/"]:
                return False
            path_parts = path.split("/")
            return len(path_parts) == 3 and path_parts[2].isdigit() and 0 <= int(path_parts[2]) <= 0x7fffffff

        if any(pk.extkey is None for pk in multisig.pubkeys):
            raise BadArgumentError("Ledger Bitcoin app requires full extended keys for multisig")
        ...
        def format_key_info(pubkey: PubkeyProvider):
